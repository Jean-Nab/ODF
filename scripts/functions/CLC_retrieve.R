                       ...
                       prefixe_fichier)
{
  ...
  # EXTRACTION ----
    ...
    # Buffer large -------
      ...
      vec.iteration_larg <- seq(from = 1, to = length(PointBuffLarg$ID_extract), by = 250) # argument suplementaire ? / add condition : NA/all -> tous
      ...
      for(i in vec.iteration_larg){
        ...
        PointBuffLarg.tmp <- PointBuffLarg[i:min((i+249),length(PointBuffLarg$ID_extract)),] # si add condition --> condition - 1
